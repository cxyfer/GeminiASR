...
> [!IMPORTANT]
> 虽然 `gemini-3-pro-preview` 以及 `gemini-3-flash-preview` 已经推出，但在目前使用的 prompt template 下，对时间戳的判断远不如 `gemini-2.5-pro` 甚至是 `gemini-2.5-flash`，因此综合考量还是推荐使用 `gemini-2.5-flash` 模型。
