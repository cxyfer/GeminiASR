...
class ConfigManager:
    ...
    def _ensure_env_loaded(self):
        ...
        if not self._env_loaded:
            try:
                ...
                self.logger.debug("成功載入 .env 文件")
            except ImportError:
                # 如果 python-dotenv 未安裝，靜默忽略
                self.logger.debug("python-dotenv 未安裝，跳過 .env 載入")
                pass
    ...
    def get_api_keys(self) -> List[str]:
        ...
        if env_keys_str:
            env_keys = [key.strip()
                        for key in env_keys_str.split(',') if key.strip()]
        ...
        if toml_keys:
            ...
            self.logger.debug("API 金鑰來源：TOML 配置檔案")
            return toml_keys
        elif env_keys:
            self.logger.info(f"使用環境變數中的 {len(env_keys)} 個 API 金鑰")
            self.logger.debug("API 金鑰來源：環境變數（含 .env）")
            return env_keys
        else:
    ...
    def get_base_url(self) -> str:
        ...
        if env_base_url:
            self.logger.debug(f"BASE_URL 來源：環境變數（含 .env）- {env_base_url}")
            return env_base_url
        ...
        if 'advanced' in config and 'base_url' in config['advanced']:
            ...
            self.logger.debug(f"BASE_URL 來源：TOML 配置檔案 - {base_url}")
            return base_url

        # 3. 使用預設值
        default_url = 'https://generativelanguage.googleapis.com/'
        self.logger.debug(f"BASE_URL 來源：預設值 - {default_url}")
        return default_url
    ...
    def log_config_details(self, config: Dict[str, Any], logger):
        ...
        # 顯示配置來源信息
        self._log_config_sources(logger)
    ...
    def _log_config_sources(self, logger):
        """
        記錄配置來源信息
        ...
        # 記錄 API 金鑰來源
        if toml_keys:
            logger.debug("API 金鑰來源：TOML 配置檔案")
        elif env_keys:
            logger.debug("API 金鑰來源：環境變數（含 .env）")
        else:
            logger.debug("API 金鑰來源：無")
        
        # 記錄 BASE_URL 來源
        env_base_url = os.getenv('BASE_URL')
        if env_base_url:
            logger.debug(f"BASE_URL 來源：環境變數（含 .env）- {env_base_url}")
        elif 'advanced' in config and 'base_url' in config['advanced']:
            logger.debug(f"BASE_URL 來源：TOML 配置檔案 - {config['advanced']['base_url']}")
        else:
            logger.debug("BASE_URL 來源：預設值 - https://generativelanguage.googleapis.com/")
